		...
		<ul class="pagination">
			...
			<c:choose>
				<c:when test="${paging.blockStart > 1 }">
					<li><a href="<%=request.getContextPath() %>/board/freelist?cPage=${paging.blockStart-1}"> <i class="fas fa-angle-left"></i> </a></li>
				</c:when>
				<c:otherwise>
					<li><a href="<%=request.getContextPath() %>/board/freelist?cPage=${paging.blockStart}"> <i class="fas fa-angle-left"></i> </a></li>
				</c:otherwise>
			</c:choose>
...
<%-- 				<li class="active"><a href="<%=request.getContextPath() %>/board/freelist?cPage=${page}"><span>${page }</span></a></li> --%>
			...
			<c:choose>
				<c:when test="${paging.blockEnd+1 > paging.lastPage }">
					<li><a href="<%= request.getContextPath() %>/board/freelist?cPage=${paging.blockEnd}"><i class="fas fa-angle-right"></i></a></li>
				</c:when>
				<c:otherwise>
					<li><a href="<%= request.getContextPath() %>/board/freelist?cPage=${paging.blockEnd+1}"><i class="fas fa-angle-right"></i></a></li>
				</c:otherwise>
			</c:choose>
